...
ActivateAI = function()
	...
	Trigger.OnProduction(CHeavyFactory2, function (producer, produced)
		if produced.Type == "combat_tank_h" and producer.Owner.IsBot then
			AICrushLogic(produced, producer.Owner)
		end
	end)
	Trigger.OnProduction(CHeavyFactory1, function (producer, produced)
		if produced.Type == "combat_tank_h" and producer.Owner.IsBot then
			AICrushLogic(produced, producer.Owner)
		end
	end)
	Trigger.OnProduction(CStarport, function (producer, produced)
		if produced.Type == "combat_tank_h.starport" and producer.Owner.IsBot then
			AICrushLogic(produced, producer.Owner)
		end
	end)
	Trigger.OnProduction(AHeavyFactory1, function (producer, produced)
		if produced.Type == "combat_tank_a" and producer.Owner.IsBot then
			AICrushLogic(produced, producer.Owner)
		end
	end)
	Trigger.OnProduction(AHeavyFactory2, function (producer, produced)
		if produced.Type == "combat_tank_a" and producer.Owner.IsBot then
			AICrushLogic(produced, producer.Owner)
		end
	end)
	Trigger.OnProduction(AStarport, function (producer, produced)
		if produced.Type == "combat_tank_a.starport" and producer.Owner.IsBot then
			AICrushLogic(produced, producer.Owner)
		end
	end)
end
